...
import io.hanbings.cynops.database.interfaces.MySqlData;
import io.hanbings.cynops.database.interfaces.MySqlDataTable;
...
public class MySqlSqlBuilder {
    ...
    public static String createTable(Class<?> table) {
        ...
        if (table.isAnnotationPresent(MySqlDataTable.class)) {
            // 获取表名
            builder.append(table.getAnnotation(MySqlDataTable.class).table()).append(" (");
            ...
            for (Field field : fields) {
                // 扫描有效字段
                if (field.isAnnotationPresent(MySqlData.class)) {
                    // 添加分隔符
                    if (isAdd) {
                        builder.append(", ");
                    } else {
                    ...
                    // 获取注解
                    MySqlData data = field.getAnnotation(MySqlData.class);
                    // 字段名 类型 主键 唯一约束 自增 非空
                    builder.append(table.getAnnotation(MySqlDataTable.class).isToUpper()
    ...
    public static String deleteTable(Class<?> table) {
        ...
        if (table.isAnnotationPresent(MySqlDataTable.class)) {
            builder.append(table.getAnnotation(MySqlDataTable.class).table());
        }
        return builder.append(";").toString();
    }
    ...
    public static String create(Object data) {
        ...
        if (data.getClass().isAnnotationPresent(MySqlDataTable.class)) {
            builder.append(data.getClass().getAnnotation(MySqlDataTable.class).table());
            // 遍历字段 获取符合要求的字段
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(MySqlData.class)) {
                    ...
                    builder.append(data.getClass().getAnnotation(MySqlDataTable.class).isToUpper()
                            ? getColumnName(field.getName()).toUpperCase(Locale.ROOT) : getColumnName(field.getName()));
                }
            }
    ...
    public static String update(Object data) {
        ...
        if (data.getClass().isAnnotationPresent(MySqlDataTable.class)) {
            ...
            for (Field field : fields) {
                ...
                if (field.isAnnotationPresent(MySqlData.class)) {
                    if (data.getClass().getAnnotation(MySqlDataTable.class).isToUpper()) {
                    ...
                    try {
                        ...
                        if (field.getAnnotation(MySqlData.class).isPrimaryKey()) {
        ...
        if (data.getClass().isAnnotationPresent(MySqlDataTable.class)) {
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(MySqlData.class)) {
                    if (field.getAnnotation(MySqlData.class).isPrimaryKey()) {
                        builder.append(" FROM ")
                                .append(data.getClass().getAnnotation(MySqlDataTable.class).table())
                                .append(" WHERE ")
                                .append(data.getClass()
                                        .getAnnotation(MySqlDataTable.class).isToUpper() ? field.getName().toUpperCase(Locale.ROOT) : field.getName())
                                .append(" = ");
