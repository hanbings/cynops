...
1. 类名大写驼峰 方法名小写驼峰 **禁止使用匈牙利命名法** 即类型缩写
2. 包名全小写 包名类名方法名**使用复数**
3. 同功能不同参数的方法 **优先使用方法重载** 如果确实无法进行重载 则在功能后使用 with 连接词 **功能 + With + 参数类型**
4. 同功能不同参数也不同返回值 使用 **功能 + To + 返回值类型** 如 base64ToBytes() 需与上一条规则连用 如 base64WithMimeToBytes() 当然 能够使用重载完成的方法还是优先使用重载
